#!/bin/bash
# Launch an MCP server module from the imlab_mcp package.
# Usage: run.sh <module>   e.g. run.sh imlab_mcp.hpc_server
set -euo pipefail
...
if ! "$VENV/bin/python" -c "import imlab_mcp, mcp, remotemanager" >/dev/null 2>&1; then
