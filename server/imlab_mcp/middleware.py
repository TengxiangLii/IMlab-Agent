...
- Commands run under a login shell (Slurm on the im cluster resolves its
...
def _relax_rsync_version_check() -> None:
    """Disable remotemanager's rsync version gate.

    Constructing any remotemanager URL/Computer instantiates a default rsync
    transport and version-checks it at import time; stock macOS ships rsync
    2.6.9, below remotemanager's required 3.0.0, which would raise before we ever
    connect. We only ever use `Computer.cmd` (a direct SSH exec) — rsync is never
    used to stage files here — so the transport's rsync version is irrelevant.
    Lower the library's threshold constant (a documented, soft check) rather than
    forcing every macOS user to upgrade rsync. Best-effort: never let this break
    startup if the library internals change.
    """
    try:
        import sys

        from remotemanager.transport.rsync import rsync as _rsync_cls
        module = sys.modules[_rsync_cls.__module__]
        if hasattr(module, "RSYNC_MIN_VERSION"):
            module.RSYNC_MIN_VERSION = "0.0.0"
    except Exception:
        pass


_relax_rsync_version_check()
...
def get_frontend() -> Computer:
    """The (cached) Computer targeting the im cluster login node.

    `timeout` is remotemanager's per-attempt communication window (default 5s);
    raise it so large scheduler outputs (e.g. sacct on an account with a long job
    history) or a slower/proxied SSH link don't spuriously fail.
    """
    return Computer(
        template="#!/bin/bash -l",
    ...
    with contextlib.redirect_stdout(sys.stderr):
        try:
            ...
            if not config.CONFIG_PATH.exists():
                raise RuntimeError(
                    "Plugin not configured — run the 'imlab-configuring' skill to "
